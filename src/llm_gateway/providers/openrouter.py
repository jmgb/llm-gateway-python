...
class OpenRouterAdapter:
    ...
    def _build_messages(self, request: LLMRequest) -> list[dict[str, str]]:
        ...
        Whether the route behind the request enforces a schema is unknowable
        from here, so the shape is described rather than assumed: a model that
        does enforce it reads a redundant instruction, and one that does not
        reads its only specification.
        """
